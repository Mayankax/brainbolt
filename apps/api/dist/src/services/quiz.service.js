...
async function getQuestionPool(difficulty) {
...
export async function submitAnswer(params) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (state.stateVersion !== stateVersion) {
            const error = new Error("State version mismatch");
        ...
        // ✅ Check Correctness
        const correct = crypto.createHash("sha256").update(answer).digest("hex") ===
        ...
        // 🏆 Update Leaderboards
        await updateLeaderboards(userId, updatedState.totalScore, updatedState.maxStreak);
